...
public struct DatabaseRegion: Sendable {
    private let tableRegions: [String: TableRegion]? // nil means "full database"
    ...
    func intersection(_ other: DatabaseRegion) -> DatabaseRegion {
        ...
        guard let otherTableRegions = other.tableRegions else {
            return self
        }
        ...
        for (table, tableRegion) in tableRegions {
            ...
            guard !tableRegionIntersection.isEmpty else {
                continue
            }
    ...
    func observableRegion() throws -> DatabaseRegion {
    ...
    private func isSQLiteInternalTable(_ tableName: String) -> Bool {
...
extension DatabaseRegion {
    ...
    /// database were modified by an event of this operation.
    public func isModified(by operation: DatabaseEventOperation) -> Bool {
        intersection(operation.modifiedRegion).isEmpty == false
    }
    ...
    public func isModified(by event: SQLiteUpdateEvent) -> Bool {
        ...
        guard let tableRegion = tableRegions[event.table] else {
...
public protocol DatabaseRegionConvertible: Sendable {
    ...
    func databaseRegion() throws -> DatabaseRegion
}
...
extension DatabaseRegion: DatabaseRegionConvertible {
    public func databaseRegion() throws -> DatabaseRegion {
        self
    }
}
...
public struct AnyDatabaseRegionConvertible: DatabaseRegionConvertible {
    let _region: @Sendable () throws -> DatabaseRegion

    public init(_ region: @escaping @Sendable () throws -> DatabaseRegion) {
        _region = region
    }

    public init(_ region: some DatabaseRegionConvertible) {
        _region = { try region.databaseRegion() }
    }
...
extension DatabaseRegion {
    ...
    static func union(_ regions: [any DatabaseRegionConvertible]) -> @Sendable () throws -> DatabaseRegion {
        {
            try regions.reduce(into: DatabaseRegion()) { union, region in
                try union.formUnion(region.databaseRegion())
            }
        }
    }
}
